...
const goAhead = (i) => {
    setTimeout( () => {
        let distanceX = Math.abs(positionY - coinPositionY);
        let distanceY = Math.abs(positionX - coinPositionX);
...
const tailMove = () => {
    ...
    tail[4].style.left = snakeRouteX[steps-1];
    tail[4].style.top = snakeRouteY[steps-1];
    if (score > 1) {
        for (let i = 0; i < score-1; ++i) {
            tail[i+5].style.left = snakeRouteX[steps-i*10-2];
            tail[i+5].style.top = snakeRouteY[steps-i*10-2];
        }
    }
};
    ...
    for (let i = 0; i < 5; ++i) {
        if (score > highScores[i]) {
            ...
            for (let j = i; j < 4; ++j) {
                ...
                highScores[j+1] = term;
                term = names[j+1].innerHTML;
                names[j+1].innerHTML = name[j].innerHTML;
                names[j].innerHTML = term;
            }
            highScores[i] = score;
